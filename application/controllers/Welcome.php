...
protected function uploadPhoto(array &$data)
{
    ...
    public function profile() {
        // Load model untuk mengakses data pengguna
        $this->load->model('MSudi');

        // Mendapatkan data pengguna dari model (contoh saja, ganti dengan metode yang sesuai)
        $UserID = 1; // ID pengguna yang sedang login, dapat disesuaikan
        $user = $this->MSudi->getUsername($UserID);

        // Kirim data pengguna ke view
        $data['user'] = $user;

        // Load view profile.php dengan data pengguna
        $this->load->view('depan/V_Profile', $data);
    }
}
